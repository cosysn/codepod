...
│  │  │              域名规则                    │   │   │
│  │  │                                        │   │   │
│  │  │  格式: {sandbox-id}.{workspace}.local │   │   │
│  │  │  示例: abc123.dev.codepod.local        │   │   │
│  │  │                                        │   │   │
│  │  │  - 自动分配域名                        │   │   │
│  │  │  - 自定义域名 (可选)                   │   │   │
│  │  │  - 通配符支持                         │   │   │
...
│  │                                                  │   │
│  │  1. 配置 DNS / etc/hosts                       │   │
│  │     *.codepod.local -> 127.0.0.1 (或 Server IP)│   │
│  │                                                  │   │
│  │  2. 访问 Sandbox                                │   │
│  │     https://abc123.dev.codepod.local:2222      │   │
│  │                                                  │   │
│  └─────────────────────┬───────────────────────────┘   │
│                        │                                 │
│                        │ HTTP/SSH                        │
...
1. Sandbox 创建时:
   Runner ──► Server: 注册域名
              │
              ▼
   Server DNS ──► 添加 DNS 记录
              │
              ▼
   {sandbox-id}.{workspace}.codepod.local ──► Container IP

2. 域名解析时:
   本地电脑 ──► DNS 查询 ──► codepod.local
              │
              ▼
   返回 Sandbox IP

3. Sandbox 间通信:
   Sandbox A ──► ping xyz789.dev.codepod.local
              │
              ▼
   DNS 解析 ──► 172.17.0.3 ──► Sandbox B
```
...
{
  "type": "dev-container",
  "workspace": "dev",           # 工作空间名称
  "domain": {
    "custom": "myapp",         # 自定义域名 (可选)
    "suffix": "codepod.local"   # 域名后缀
  },
  "network": {
    "mode": "shared",          // 隔离/shared
    "dns": true                // 启用 DNS
  }
}

# 生成的域名:
# - 默认: abc123.dev.codepod.local
# - 自定义: myapp.dev.codepod.local
```
